package eapearson_ReadsSetEditor::eapearson_ReadsSetEditorClient;
...
eapearson_ReadsSetEditor::eapearson_ReadsSetEditorClient
...
sub new
{
    ...
    my $self = {
	client => eapearson_ReadsSetEditor::eapearson_ReadsSetEditorClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is an eapearson_ReadsSetEditor.save_read_set_params
$return is an eapearson_ReadsSetEditor.save_read_set_output
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "eapearson_ReadsSetEditor.save_read_set",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "eapearson_ReadsSetEditor.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "eapearson_ReadsSetEditor.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for eapearson_ReadsSetEditor::eapearson_ReadsSetEditorClient\n";
    }
    if ($sMajor == 0) {
        warn "eapearson_ReadsSetEditor::eapearson_ReadsSetEditorClient version is $svr_version. API subject to change.\n";
    }
}
...
package eapearson_ReadsSetEditor::eapearson_ReadsSetEditorClient::RpcClient;
